        ...
        <v-card rounded="xl" class="mb-4">
          ...
          <v-card-text class="pa-4">
            <div v-for="(svc, i) in form.services" :key="i" class="d-flex gap-2 mb-3 align-center">
              ...
              />
              <v-text-field v-model.number="svc.quantity" label="Soni" type="number" variant="outlined" density="compact" hide-details style="max-width: 80px;" @update:model-value="calcTotal" />
              <v-text-field v-model.number="svc.price" label="Narx" type="number" variant="outlined" density="compact" hide-details style="max-width: 120px;" @update:model-value="calcTotal" />
...
function calcTotal() {} // Reactive — computed handles it
...
async function saveVisit(printReceipt) {
  ...
  try {
    ...
    if (printReceipt) {
      // Open receipt preview in new tab
